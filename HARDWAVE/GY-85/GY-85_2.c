...
unsigned char I2C_ReadByte_2(void)  //数据从高位到低位//
{ 
    u8 i=8;
    u8 ReceiveByte=0;
...
int Single_Write_2(unsigned char SlaveAddress,unsigned char REG_Address,unsigned char REG_data)		     //void
{
  	if(!I2C_Start_2())
			return 0;
    I2C_SendByte_2(SlaveAddress);   //发送设备地址+写信号//I2C_SendByte(((REG_Address & 0x0700) >>7) | SlaveAddress & 0xFFFE);//设置高起始地址+器件地址 
    if(!I2C_WaitAck_2())
		{
			I2C_Stop_2();
			return 0;
		}
    I2C_SendByte_2(REG_Address );   //设置低起始地址      
    I2C_WaitAck_2();	
...
{  
	  ...
	  if(!I2C_Start_2())
			return 0;
    I2C_SendByte_2(SlaveAddress); //I2C_SendByte(((REG_Address & 0x0700) >>7) | REG_Address & 0xFFFE);//设置高起始地址+器件地址 
    if(!I2C_WaitAck_2())
		{
			I2C_Stop_2();
//			test=1; 
    ...
    I2C_SendByte_2((u8) REG_Address);   //设置低起始地址      
    I2C_WaitAck_2();
    I2C_Start_2();
...
{
   Single_Write_2(ADXL345_Addr,0x31,0x0B);   //测量范围,正负16g，13位模式
 
   Single_Write_2(ADXL345_Addr,0x2D,0x08);   //选择电源模式   参考pdf24页
   Single_Write_2(ADXL345_Addr,0x2E,0x80);   //使能 DATA_READY 中断
